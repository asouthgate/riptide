...
fn cal_acceleration_ij(
    ...
    // vterm.0 *= muij / rhoij;
    // vterm.1 *= muij / rhoij;
    let gradk = cubic_spline_grad(dx, dy, r, 0.5 * hij * rhoij.powf(-0.5));
    ...
    (-mj * pterm * gradk.0, -mj * pterm * gradk.1)
}
...
mod tests {
    ...
    fn cal_acceleration_ij_trivial() {
        ...
        println!("??{} {}", a.0, a.1);
        assert!(a.0 < 0.0);
        assert!(a.1.abs() < 0.00000001); // sometimes we get signed zero for reasons
    ...
    fn test_cal_acceleration_i_justpressure_symmetry() {
        ...
        cal_acceleration_i(0, &mut nbrs, 1.0, 0.0);
    ...
    fn test_quad_in_box() {
        ...
        for _it in 0..500 {
            ...
            for k in 0..n_real_particles {
                cal_acceleration_i(k, &mut particles, h, 0.0);
            }
            for k in 0..n_real_particles {
                let i = particles[k].get_y().trunc() as usize;
                let j = particles[k].get_x().trunc() as usize;
